...
./replace.sh $INSTALL_DIR/serverconfig.xml "My Game Host" "SANITY'S EDGE . MODDED . CUSTOM 8K MAP . EXPERIMENTAL" ServerName
...
./replace.sh $INSTALL_DIR/serverconfig.xml "A 7 Days to Die server" "An extremely modified experimental 7DTD Server, using a custom RWG-tweaked map, and over 350 new buildings. ServerTools installed. Modlets: Vanilla+, Red Eagle, Just Survive. No client side installation nessesary." ServerDescription
...
./replace.sh $INSTALL_DIR/serverconfig.xml 'value=""' 'value="This is an experimental server. Do NOT use it as your everyday server. It is still under heavy development."' ServerLoginConfirmationText
